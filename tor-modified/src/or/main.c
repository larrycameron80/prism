...
run_scheduled_events(time_t now)
{
  ...
  static time_t time_to_write_passive_bridge_info = 0;
  static time_t time_to_change_hmack_key_passive_bridge_info = 0;
  static time_t time_to_check_port_forwarding = 0;
  static time_t time_to_launch_reachability_tests = 0;
  static int should_init_bridge_stats = 1;
  static int should_init_bridge_passive_info = 1;
  static int should_init_hmac_key_change_bridge_passive_info = 1;
...
#define CHECK_WRITE_STATS_INTERVAL (60*60)
  ...
  /**
	* Added By Behrooz
	* 2013-06-30  6:36:11 PM
	* Purpose: schedule passive info writes
	**/
	if (time_to_write_passive_bridge_info < now) {
		if (should_init_bridge_passive_info) {
			/* (Re-)initialize bridge passive info. */
			geoip_bridge_passive_info_init(now);
			time_to_write_passive_bridge_info = now + WRITE_BRIDGE_PASSIVE_INFO_INTERVAL;
			should_init_bridge_passive_info = 0;
		} else {
			/* Possibly write bridge statistics to disk and ask when to write
			 * them next time. */
			time_to_write_passive_bridge_info = geoip_bridge_passive_info_write(
																		time_to_write_passive_bridge_info);
		}
	}
	...
	* Purpose: schedule passive info writes
	**/
	if (time_to_change_hmack_key_passive_bridge_info < now) {
		if (should_init_hmac_key_change_bridge_passive_info) {
			/* (Re-)initialize HMAC key change bridge passive info. */
			geoip_init_hmac(now);
			should_init_hmac_key_change_bridge_passive_info = 0;
			time_to_change_hmack_key_passive_bridge_info = now+HMAC_KEY_CHANGE_BRIDGE_PASSIVE_INFO_INTERVAL;
		}
		else
		{
      //Change Key
      time_to_change_hmack_key_passive_bridge_info = geoip_change_hmac_key(
		                                               time_to_change_hmack_key_passive_bridge_info);
		}
