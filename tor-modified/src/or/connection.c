...
//  log_fn(LOG_DEBUG,"read_to_buf returned %d.",read_result);
  ...
  if (n_read > 0) {
    ...
    *max_to_read = at_most - n_read;
    ...
    /**
		* Added By Behrooz
		* 2013-06-29  6:58:15 PM
		* Purpose: To Keep track of total read bytes
		**/
		if (n_read>0 && conn->type == CONN_TYPE_OR) {
			or_connection_t *or_conn = TO_OR_CONN(conn);
			if(!connection_or_nonopen_was_started_here(or_conn))
				geoip_passive_note_read_data(or_conn,n_read);
		}
...
connection_handle_write_impl(connection_t *conn, int force)
{
  ...
  /**
	* Added By Behrooz
	* 2013-06-29  6:57:51 PM
	* Purpose: To keep track of total written bytes
	**/
  if (n_written>0 && conn->type == CONN_TYPE_OR) {
		or_connection_t *or_conn = TO_OR_CONN(conn);
		if(!connection_or_nonopen_was_started_here(or_conn))
			geoip_passive_note_write_data(or_conn,n_written);
	}
