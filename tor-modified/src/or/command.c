...
#define WARN_HANDOFF_FAILURE_INTERVAL (6*60*60)
  ...
  /**
	* Added By Behrooz
	* 2013-06-29  2:27:13 PM
	* Purpose: Received the 'create' cell from the client
	* meaning that the client want to build a circuit through us!
	**/
	if(!connection_or_nonopen_was_started_here(conn))
		geoip_passive_note_create_cell_event(conn);
}
...
command_process_relay_cell(cell_t *cell, or_connection_t *conn)
{
  ...
  /**
	* Added By Behrooz
	* 2013-06-29  3:34:24 PM
	* Purpose: To see if our clients transmitt data and use relay cells or not!!
	**/
  if (direction == CELL_DIRECTION_OUT)/**< The cell is moving away through us*/
  {
  	geoip_passive_note_relay_cell_event(conn);
  }
}
...
command_process_versions_cell(var_cell_t *cell, or_connection_t *conn)
{
  ...
  /**
  	* Added By Behrooz
  	* 2013-06-29  11:23:37 AM
  	* Purpose: to see if clients do send version cells
  	**/
  if(!started_here)
  	geoip_passive_note_version_cell_event(conn,1);
  ...
  switch (conn->_base.state)
    ...
    default:
      ...
      /**
			* Added By Behrooz
			* 2013-06-29  11:43:56 AM
			* Purpose: Client sent VERSION cell while in unexpected state!
			**/
      if(!started_here)
      	geoip_passive_note_version_cell_event(conn,3);
      return;
  }
  ...
  if (!highest_supported_version) {
    ...
    /**
		...
		* Purpose: No common version between mine and the client!!
		**/
    if(!started_here)
    	geoip_passive_note_version_cell_event(conn,4);
    return;
    ...
    /**
		* Added By Behrooz
		* 2013-06-29  11:42:45 AM
		* Purpose: version 1 makes no sense, since version 1 has no VERSIONS cells
		**/
		if(!started_here)
			geoip_passive_note_version_cell_event(conn,5);
    connection_mark_for_close(TO_CONN(conn));
    return;
  ...
  /**
	* Added By Behrooz
	* 2013-06-29  12:12:48 PM
	* Purpose: Client completed sending a valid Version
	**/
  if(!started_here)
		geoip_passive_note_version_cell_event(conn,2);
