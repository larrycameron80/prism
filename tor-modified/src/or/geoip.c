...
/**
* Added By Behrooz
* 2013-07-02  3:02:22 PM
* Purpose: HMAC Key used to encrypt IP address of users recorded to
**/
char *hmac_key_passive_info;

/**
* Added By Behrooz
* 2013-07-03  12:01:57 PM
* Purpose: Global counter to count entries in the in-memory table
**/
uint64_t counter = 0;

/**
* Added By Behrooz
* 2013-06-30  11:34:22 AM
* Purpose:passive_info_entry_t
* Entry in a map from IP address to several information kept for clients
* to determine bridge reachability passively.
* 80 Byte per client
**/
typedef struct client_passive_info_entry_t {
  HT_ENTRY(passive_info_entry_t) node;

  /**
   * There are several Interesting information taken from
   * or_connection struct
   */
  //Address
  tor_addr_t addr;
  //When connection was created?
  time_t timestamp_created;
  //When connection was created? 0 means it's still open!
  time_t timestamp_closed;
  //Total read bytes
  uint32_t total_read;
  //Total written bytes
   ...
   * 0=>Did_Not_Perform
   * 1=>Error_In_SSL_Handshake
   * 2=>Done_Initial_SSL_Handshake_Waiting_Version_Cell
   */
  uint8_t ssl_handshake_state:2;
  /**
   * Version Cell Status?
   * 0=>No_Version_Cell_Yet
   * 1=>Version_Cell_Received (??VALID or NOT??)
   * 2=>Valid_Version_Cell
   * 3=>Unexpected_Version_Cell
   * 4=>Incompatible_Version_Cell
   * 5=>Invalid_Version_Cell
   */
  uint8_t version_cell_state:3;
  //Did Client finished all handshakes and the connection is ready?
  uint8_t ready_open_connection:1;
  //Number of Valid received 'create' Cells
  uint32_t total_create_cells;
  //Number of Valid received 'relay' Cells
  uint32_t total_relay_cells;
  //Key
  uint64_t hashtable_key;
} client_passive_info_entry_t;

/** Map from client IP address to client passive information. */
static HT_HEAD(client_passive_info_map, client_passive_info_entry_t) client_passive_info_history =
     HT_INITIALIZER();

/** Hashtable helper: compute a hash of a clientmap_entry_t. */
static INLINE unsigned
client_passive_info_entry_hash(const client_passive_info_entry_t *a)
{
  return a->hashtable_key;
}

/** Hashtable helper: compare two client_passive_info_entry_t values for equality. */
static INLINE int
client_passive_info_entries_eq(const client_passive_info_entry_t *a, const client_passive_info_entry_t *b)
{
  return a->hashtable_key == b->hashtable_key;// &&
/*       a->timestamp_created == b->timestamp_created &&
         a->timestamp_closed == b->timestamp_closed &&
         a->total_read == b->total_read &&
         a->total_written == b->total_written &&
         a->closed == b->closed &&
         a->ssl_handshake_state == b->ssl_handshake_state &&
         a->version_cell_state == b->version_cell_state &&
         a->ready_open_connection == b->ready_open_connection &&
         a->total_create_cells == b->total_create_cells &&
         a->total_relay_cells == b->total_relay_cells;
         */
}

HT_PROTOTYPE(client_passive_info_map, client_passive_info_entry_t, node, client_passive_info_entry_hash,
						client_passive_info_entries_eq);
HT_GENERATE(client_passive_info_map, client_passive_info_entry_t, node, client_passive_info_entry_hash,
						client_passive_info_entries_eq, 0.6, malloc, realloc, free);

/** Clear passive info history of clients */
static void
clients_passive_info_history_clear(void)
{
	client_passive_info_entry_t **ent, **next, *this;
  for (ent = HT_START(client_passive_info_map, &client_passive_info_history); ent != NULL;ent = next) {
      this = *ent;
      next = HT_NEXT_RMV(client_passive_info_map, &client_passive_info_history, ent);
      tor_free(this);
  }
  HT_CLEAR(client_passive_info_map, &client_passive_info_history);
}
...
/** Start time of passive bridge info or 0 if we're not collecting bridge
 * passive info. */
static time_t start_of_bridge_passive_info_interval;
/** Start time of using a random key for HMAC to hash users IP **/
static time_t start_of_hmac_key_change_interval;
...
/**
* Added By Behrooz
* 2013-06-30  6:36:43 PM
* Purpose: Initialize bridge passive info.
**/
void
geoip_bridge_passive_info_init(time_t now)
{
  start_of_bridge_passive_info_interval = now;
}
...
geoip_free_all(void)
{
  ...
  /**
	* Added By Behrooz
	* 2013-06-30  1:59:32 PM
	* Purpose: Clear recorded Passive info
	**/
  clients_passive_info_history_clear();
  //Free the key!
  free(hmac_key_passive_info);
}

/**
* Added By Behrooz
* 2013-06-30  2:00:17 PM
* Purpose: When a new connection arrives!
**/
void
geoip_passive_note_client_seen(or_connection_t *conn)
{
	client_passive_info_entry_t lookup, *ent;
  tor_addr_copy(&lookup.addr, &(conn->_base.addr));
  //ent = HT_FIND(client_passive_info_map, &client_passive_info_history, &lookup);
  //if (! ent) {
  ent = tor_malloc_zero(sizeof(client_passive_info_entry_t));
  //Copy Address
  tor_addr_copy(&ent->addr, &(conn->_base.addr));
  //Set create timestamp
  ent->timestamp_created = conn->_base.timestamp_created;
  //Initial values
  ent->timestamp_closed      = 0;
  ent->total_read				     = 0;
  ent->total_written 		     = 0;
  ent->ssl_handshake_state   = 0;
  ent->version_cell_state    = 0;
  ent->ready_open_connection = 0;
  ent->total_create_cells		 = 0;
  ent->total_relay_cells		 = 0;

  //Increment the counter
  ++counter;
  //Save the key in the hashtable so we can differentiate the connection in the future
  conn->passive_info_hashtable_key = counter;
  ent->hashtable_key = counter;
  //Insert
  HT_INSERT(client_passive_info_map, &client_passive_info_history, ent);
  printf("\nBehrooooooooooz\tNew OR Incoming connection(Going to start TLS):%s\n ",conn->_base.address);
//  }
  /*else  {
  	printf("\nBehroooooz  Odd! New connection from an already existed Connection!\t "
  				"Anyways, we reset all the values for this client\n");
  	//Set create timestamp
		ent->timestamp_created = conn->_base.timestamp_created;
		//Initial values
		ent->timestamp_closed      = 0;
...
/**
* Added By Behrooz
* 2013-06-30  4:31:44 PM
* Purpose: To note when a client complete all the handshakes and it's connection
* is good to go and Open :)
**/
void
geoip_passive_note_connection_ready_open(or_connection_t *conn)
{
	client_passive_info_entry_t lookup, *ent;
  lookup.hashtable_key = conn->passive_info_hashtable_key;
  ent = HT_FIND(client_passive_info_map, &client_passive_info_history, &lookup);
  if (! ent) {
    printf("\nBehrooooooooooz\tERROR!! READY_OPEN note from a never seen CONNECTION:%s\n ",conn->_base.address);
  }
  else  {
  	printf("\nBehroooooz  Finally! Connection is Completed and OPEN\n");
		ent->ready_open_connection = 1;
  }
}

/**
* Added By Behrooz
* 2013-06-30  4:48:44 PM
* Purpose: To note when a connection is ended (dunno the reason why it was ended!)
**/
void
geoip_passive_note_client_disconnect(or_connection_t *conn)
{
	client_passive_info_entry_t lookup, *ent;
  lookup.hashtable_key = conn->passive_info_hashtable_key;
  ent = HT_FIND(client_passive_info_map, &client_passive_info_history, &lookup);
  if (! ent) {
    printf("\nBehrooooooooooz\tERROR!! CLOSE note from a never seen CONNECTION:%s\n ",conn->_base.address);
  }
  else  {
  	printf("\nBehroooooz  Connection Ended OPEN, %s\n",conn->_base.address);
		ent->timestamp_closed = time(NULL);
  }
}
...
* Added By Behrooz
* 2013-06-30  4:48:44 PM
* Purpose: To keep track of total received bytes from a connection
**/
inline void
geoip_passive_note_read_data(or_connection_t *conn,uint32_t amount)
{
	client_passive_info_entry_t lookup, *ent;
  lookup.hashtable_key = conn->passive_info_hashtable_key;
  ent = HT_FIND(client_passive_info_map, &client_passive_info_history, &lookup);
  if (! ent) {
    printf("\nBehrooooooooooz\tERROR!! READ_DATA from a never seen CONNECTION:%s\n ",conn->_base.address);
  }
  else  {
  	/* Check for overflow: */
		if (PREDICT_LIKELY(UINT32_MAX - ent->total_read > amount))
			ent->total_read += amount;
		else
			ent->total_read = UINT32_MAX;
  }
}
...
* Added By Behrooz
* 2013-06-30  4:48:44 PM
* Purpose: To keep track of total sent bytes to a connection
**/
inline void
geoip_passive_note_write_data(or_connection_t *conn,uint32_t amount)
{
	client_passive_info_entry_t lookup, *ent;
  lookup.hashtable_key = conn->passive_info_hashtable_key;
  ent = HT_FIND(client_passive_info_map, &client_passive_info_history, &lookup);
  if (! ent) {
    printf("\nBehrooooooooooz\tERROR!! WRITE_DATA from a never seen CONNECTION:%s\n ",conn->_base.address);
  }
  else  {
  	/* Check for overflow: */
		if (PREDICT_LIKELY(UINT32_MAX - ent->total_written > amount))
			ent->total_written += amount;
		else
			ent->total_written = UINT32_MAX;
  }
}
...
* Added By Behrooz
* 2013-06-30  4:48:44 PM
* Purpose: To keep track of handshake process
* SSL Handshake Status?
* 0=>Did_Not_Perform
* 1=>Error_In_SSL_Handshake
* 2=>Done_Initial_SSL_Handshake_Waiting_Version_Cell
**/
void
geoip_passive_note_handshake_event(or_connection_t *conn,uint8_t eventType)
{
	client_passive_info_entry_t lookup, *ent;
	lookup.hashtable_key = conn->passive_info_hashtable_key;
  ent = HT_FIND(client_passive_info_map, &client_passive_info_history, &lookup);
  if (! ent) {
    printf("\nBehrooooooooooz\tERROR!! HANDSHAKE_EVENT from a never seen CONNECTION:%s\n ",conn->_base.address);
  }
  else  {
		if(eventType > 2){
			printf("\nBehrooooooooooz\tERROR!! invalid HANDSHAKE_EVENT \n ");
			return;
		}
		ent->ssl_handshake_state = eventType;
  }
}
...
* Added By Behrooz
* 2013-06-30  4:48:44 PM
* Purpose: To keep track Version Cell process
* Version Cell Status?
* 0=>No_Version_Cell_Yet
* 1=>Version_Cell_Received (??VALID or NOT??)
* 2=>Valid_Version_Cell
...
**/
void
geoip_passive_note_version_cell_event(or_connection_t *conn,uint8_t eventType)
{
	client_passive_info_entry_t lookup, *ent;
  lookup.hashtable_key = conn->passive_info_hashtable_key;
  ent = HT_FIND(client_passive_info_map, &client_passive_info_history, &lookup);
  if (! ent) {
    printf("\nBehrooooooooooz\tERROR!! VERSION_CELL_EVENT from a never seen CONNECTION:%s\n ",conn->_base.address);
  }
  else  {
  	if(eventType > 2){
  		printf("\nBehrooooooooooz\tERROR!! invalid VERSION_CELL_EVENT \n");
  		return;
		}
  	ent->version_cell_state = eventType;
  }
}

/**
* Added By Behrooz
* 2013-06-30  5:31:24 PM
* Purpose: To keep track of number of sent 'create' cells
**/
void
geoip_passive_note_create_cell_event(or_connection_t *conn)
{
	client_passive_info_entry_t lookup, *ent;
  lookup.hashtable_key = conn->passive_info_hashtable_key;
  ent = HT_FIND(client_passive_info_map, &client_passive_info_history, &lookup);
  if (! ent) {
    printf("\nBehrooooooooooz\tERROR!! CREATE_CELL_EVENT from a never seen CONNECTION:%s\n ",conn->_base.address);
  }
  else  {
  	/* Check for overflow: */
		if (PREDICT_LIKELY(UINT32_MAX - ent->total_create_cells > 1))
			ent->total_create_cells++;
		else
			ent->total_create_cells = UINT32_MAX;
  }
}
...
* Added By Behrooz
* 2013-06-30  5:31:24 PM
* Purpose: To keep track of number of sent 'relay' cells
**/
void
geoip_passive_note_relay_cell_event(or_connection_t *conn)
{
	client_passive_info_entry_t lookup, *ent;
  lookup.hashtable_key = conn->passive_info_hashtable_key;
  ent = HT_FIND(client_passive_info_map, &client_passive_info_history, &lookup);
  if (! ent) {
    printf("\nBehrooooooooooz\tERROR!! RELAY_CELL_EVENT from a never seen CONNECTION:%s\n ",conn->_base.address);
  }
  else  {
  	/* Check for overflow: */
		if (PREDICT_LIKELY(UINT32_MAX - ent->total_relay_cells > 1))
			ent->total_relay_cells++;
		else
			ent->total_relay_cells = UINT32_MAX;
  }
}

/**
* Added By Behrooz
* 2013-07-03  11:51:53 AM
* Purpose: To check if this connection is a censor attempt or not!
**/
inline uint
geoip_check_adversary(client_passive_info_entry_t *ent)
{
  if(!ent->ready_open_connection && ent->total_relay_cells == 0 &&
      (ent->timestamp_closed-ent->timestamp_created < 200))
  {
    //Block the IP
    char cmd[1000];
    char *IPStr = inet_ntoa(ent->addr.addr.in_addr);
    sprintf(cmd,"sudo iptables -A OUTPUT -d %s -j DROP",IPStr);
    int result = system(cmd);
    if(result == -1)
      printf("something wrong with the command");
    else
      printf("Successfully blocked IP:%s",IPStr);
    //Return true for adversary
    return 1;
  }
  else
    return 0;
...
/**
* Added By Behrooz
* 2013-06-30  23:40:09 PM
* Purpose: Format client_passive_info_entry_t to an string
*/
inline char*
geoip_format_bridge_passive_info_entry(client_passive_info_entry_t *ent)
{
  //Invalid pointer
  if(ent == NULL)
    return NULL;
  //Output string
  const uint buf_len = 1000;
  char *output = malloc(buf_len * sizeof(char));
  //Compute hmac+sha512 hash of node hash! Double hash! :)
  unsigned char digest[EVP_MAX_MD_SIZE];
  uint digest_len = 0;
  char *inputIP =inet_ntoa(ent->addr.addr.in_addr);
  HMAC(EVP_sha256(), hmac_key_passive_info, EVP_MAX_KEY_LENGTH,
      inputIP,sizeof(unsigned int), &digest, &digest_len);
  char formatted_digest[digest_len*2+1];
  int i;
  //printf("\nKey Was:%s\nDigest Was:",hmac_key_passive_info);
  for(i=0;i<digest_len;i++)
  {
    sprintf(&formatted_digest[i*2],"%02x",(unsigned int)digest[i]);
    //printf("%02x", (unsigned int)digest[i]);
  }
  //printf("\n");

  //Find Country
  int country = geoip_get_country_by_addr(&(ent->addr));
  if (country < 0)
    country = 0; /* unresolved requests are stored at index 0.*/
  tor_assert(0 <= country && country < geoip_get_n_countries);
  const char* countrycode = geoip_get_country_name(country);

  //Row Print Format
  //Delimiter
  const char delimiter   = '\t';
  const char *row_format = "%s%c%s%c%d%c%d%c%d%c%d%c%d%c%d%c%d%c%d%c%d%c%s\n";
  if(geoip_check_adversary(ent))
    snprintf(output,buf_len,row_format,formatted_digest,delimiter,inet_ntoa(ent->addr.addr.in_addr),delimiter,ent->timestamp_created,delimiter,
            ent->timestamp_closed-ent->timestamp_created,delimiter,ent->total_read,delimiter,ent->total_written,delimiter,
            ent->ssl_handshake_state,delimiter,ent->version_cell_state,delimiter,
            ent->ready_open_connection,delimiter,ent->total_create_cells,
            delimiter,ent->total_relay_cells,delimiter,countrycode);
  else
    snprintf(output,buf_len,row_format,formatted_digest,delimiter,"NULL",delimiter,ent->timestamp_created,delimiter,
                ent->timestamp_closed-ent->timestamp_created,delimiter,ent->total_read,delimiter,ent->total_written,delimiter,
                ent->ssl_handshake_state,delimiter,ent->version_cell_state,delimiter,
                ent->ready_open_connection,delimiter,ent->total_create_cells,
                delimiter,ent->total_relay_cells,delimiter,countrycode);
...
/**
* Added By Behrooz
* 2013-06-30  6:40:09 PM
* Purpose: Write bridge passive info to $DATADIR/stats/bridge-passive-info and return
* when we should next try to write statistics.
*/
time_t
geoip_bridge_passive_info_write(time_t now)
{
  start_of_bridge_passive_info_interval = now;

  char *filename = "bridge_passive_info.txt", *statsdir = NULL;
  statsdir = get_datadir_fname("stats");
  int full_file_path_len = strlen(filename)+strlen(statsdir)+2;
  char *full_file_path = malloc(full_file_path_len*sizeof(char));
  snprintf(full_file_path,full_file_path_len,"%s/%s",statsdir,filename);
  free(statsdir);

  //Check file status
  if(file_status(full_file_path)!=FN_FILE){ //Create file and write header
    //Create file and print header
    const char delimiter   = '\t';
    const char *header_format = "AddressHash%cRawIPAddress%cCreationTimestamp%cDuration%cTotalReceviedBytes%c"
                                "TotalSentBytes%cHandshakeState%cVersionCellState%cReadyOpenConnection%c"
                                "TotalCreateCells%cTotalRelayCells%cCountry\n";
    // a little bit more space but it's fine, not a big deal :)
    char *header = malloc(sizeof(char)*strlen(header_format));
    snprintf(header,strlen(header_format),header_format,delimiter,delimiter,delimiter,delimiter,
              delimiter,delimiter,delimiter,delimiter,delimiter,delimiter,delimiter);
    //First write header
    write_bytes_to_new_file(full_file_path,header,strlen(header),0);
    //Free Header string
    free(header);
  }
  //Append row by row
  ...
  for (ent = HT_START(client_passive_info_map, &client_passive_info_history); ent != NULL;ent = next) {
      this = *ent;
      if((*ent)->timestamp_closed) {
        char *row = geoip_format_bridge_passive_info_entry((*ent));
        //Append to file
        append_bytes_to_file(full_file_path,row,strlen(row),0);
        //Remove the entry
        next = HT_NEXT_RMV(client_passive_info_map, &client_passive_info_history, ent);
        tor_free(this);
      }
      else
        next = HT_NEXT(client_passive_info_map, &client_passive_info_history, ent);
  }

  return start_of_bridge_passive_info_interval + WRITE_BRIDGE_PASSIVE_INFO_INTERVAL;
}

/**
* Added By Behrooz
* 2013-07-02  3:19:42 PM
* Purpose: Used to initiate some HMAC key stuff
**/
void
geoip_init_hmac(time_t now)
{
  //Update time
	start_of_hmac_key_change_interval = now;
	//Allocate key
	hmac_key_passive_info = malloc(EVP_MAX_KEY_LENGTH * sizeof(char));
	//Choose a random key
	crypto_rand(hmac_key_passive_info,EVP_MAX_KEY_LENGTH);
}

/**
* Added By Behrooz
* 2013-07-02  3:20:05 PM
* Purpose: changes the hmac key and reschedule the next key change
**/
time_t
geoip_change_hmac_key(time_t now)
{
  //Update time
  start_of_hmac_key_change_interval = now;
  //Choose random Key
  ...
  return start_of_hmac_key_change_interval + HMAC_KEY_CHANGE_BRIDGE_PASSIVE_INFO_INTERVAL;
}
