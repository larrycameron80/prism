...
connection_or_about_to_close(or_connection_t *or_conn)
{
  ...
  if(!connection_or_nonopen_was_started_here(or_conn))
		geoip_passive_note_client_disconnect(or_conn);
...
connection_tls_start_handshake(or_connection_t *conn, int receiving)
{
  ...
  /**
	* Added By Behrooz
	* 2013-06-28  11:30:56 AM
	* Purpose:TLS start
	**/

  int started_here = connection_or_nonopen_was_started_here(conn);
  if(started_here == 0)
  	geoip_passive_note_client_seen(conn);
...
connection_tls_continue_handshake(or_connection_t *conn)
{
 ...
 again:
  ...
  /**
		* Added By Behrooz
		* 2013-06-28  11:30:56 AM
		* Purpose:To find only Incoming Connections; In fact this is not necessary because of
		* tor_tls_is_server(conn->tls) check! but just to make sure this is an incoming connection.
	**/
	int started_here = connection_or_nonopen_was_started_here(conn);
  ...
  switch (result) {
    ...
    log_info(LD_OR,"tls error [%s]. breaking connection.",
			...
			/**
				* Added By Behrooz
				* 2013-06-28  11:30:56 AM
				* Purpose:TLS handshake Error Any
			**/
			if(started_here == 0)
				geoip_passive_note_handshake_event(conn,1);
        ...
        if (!tor_tls_is_server(conn->tls)) {
          ...
          // log_notice(LD_OR,"Done. state was %d.", conn->_base.state);
        	...
        	/**
					...
					**/
					if(started_here == 0)
						geoip_passive_note_handshake_event(conn,2);
...
connection_tls_finish_handshake(or_connection_t *conn)
{
  ...
  /**
  	* Added By Behrooz
  	* 2013-06-28  11:23:39 AM
  	* Purpose:
  	**/
  if(started_here == 0)
  	printf("\nBehroooooooz\tFinish Handshake:%s\n ",conn->_base.address);
...
connection_or_set_state_open(or_connection_t *conn)
{
  ...
  if (started_here) {
    ...
    router_set_status(conn->identity_digest, 1);
  	...
  	geoip_passive_note_connection_ready_open(conn);
