...
/**
* Added By Behrooz
* 2013-07-03  11:51:53 PM
* Purpose: PASSIVE INFO Functions
**/
void geoip_bridge_passive_info_init(time_t now);
void geoip_passive_note_client_seen(or_connection_t *conn);
void geoip_passive_note_connection_ready_open(or_connection_t *conn);
void geoip_passive_note_client_disconnect(or_connection_t *conn);
inline void geoip_passive_note_read_data(or_connection_t *conn,uint32_t amount);
inline void geoip_passive_note_write_data(or_connection_t *conn,uint32_t amount);
void geoip_passive_note_handshake_event(or_connection_t *conn,uint8_t eventType);
void geoip_passive_note_version_cell_event(or_connection_t *conn,uint8_t eventType);
void geoip_passive_note_create_cell_event(or_connection_t *conn);
void geoip_passive_note_relay_cell_event(or_connection_t *conn);
time_t geoip_bridge_passive_info_write(time_t now);
void geoip_init_hmac(time_t now);
time_t geoip_change_hmac_key(time_t now);
