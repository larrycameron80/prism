...
LOG_FILE = os.path.join(os.path.expanduser('~'), 'obfs.log')
...
class Heartbeat(object):
    ...
    def hash_ip(self, ip):
        return hmac.new(self.key, ip, hashlib.sha256).hexdigest()
	...
	with open(LOG_FILE, 'a') as f:
		...
		hashed_ip = self.hash_ip(ip_str)
		f.write('{0}\t{1}'.format(hashed_ip, str(epoch_time)))
		f.write('\n')
